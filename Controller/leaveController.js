...
// const StatusUpdate = require("../Model/statusModel");
...
const acceptLeaveRequest = async (req, res) => {
    try {
        ...
        const leaveRequest = await LeaveRequest.findByIdAndUpdate(leaveRequestId, { leaveStatus: 'accepted' }, { new: true });
        if (!leaveRequest) {
            return res.status(404).json({ message: 'Leave request not found' });
        }
        res.json({ message: 'Leave request accepted successfully', data: leaveRequest });
    } catch (error) {
...
const rejectLeaveRequest = async (req, res) => {
    try {
        const leaveRequestId = req.params.leaveRequestId;
        const leaveRequest = await LeaveRequest.findByIdAndUpdate(leaveRequestId, { leaveStatus: 'rejected' }, { new: true });
        if (!leaveRequest) {
            return res.status(404).json({ message: 'Leave request not found' });
        }
